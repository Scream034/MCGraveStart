...
# Если таймер защиты еще не прошел - запрещаем
...
# Отладка (Перевод)
tellraw @a[tag=current_looter] ["",{"text":"[Могила] Восстановление ","color":"green"},{"score":{"name":"@a[tag=current_looter,limit=1]","objective":"gs_math"},"color":"white"},{"text":" очков опыта.","color":"green"}]

# Запускаем выдачу опыта
